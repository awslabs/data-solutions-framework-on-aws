...
<Tabs>
  ...
  ```python
  # Set context value for global data removal policy
self.node.set_context("@aws-data-solutions-framework/removeDataOnDestroy", True)key = Key(self, "DataKey",
    enable_key_rotation=True,
...
<Tabs>
  ...
  ```python
  dsf.storage.AnalyticsBucket(self, "AnalyticsBucket",
    bucket_name=BucketUtils.generate_unique_bucket_name(self, "AnalyticsBucket", "my-custom-name"),
    encryption_key=key
)
  ```
