/**
 * Testing DataZone Kinesis constructs
 *
 * @group e2e/governance/datazone-kinesis
 */

import * as cdk from 'aws-cdk-lib';
import { CfnDomain } from 'aws-cdk-lib/aws-datazone';
import { TestStack } from './test-stack';
import { DataZoneKinesisAssetType } from '../../src/governance/index';
...
const mskAssetType = new DataZoneKinesisAssetType(stack, 'KinesisAssetType', {
  domainId: cfnDomain.attrId,
  removalPolicy: cdk.RemovalPolicy.DESTROY,
});
...
new cdk.CfnOutput(stack, 'KinesisAssetTypeName', {
  value: mskAssetType.kinesisCustomAssetType.name,
});
...
it('MskTopicAssetType and DataZoneMskAuthorizers created successfully', async () => {
  // THEN
  expect(deployResult.MskAssetTypeName).toContain('KinesisStreamAssetType');
});
