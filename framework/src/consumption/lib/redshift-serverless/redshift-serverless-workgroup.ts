...
import { Connections, IConnectable, Port, SecurityGroup, SelectedSubnets, SubnetSelection, Vpc } from 'aws-cdk-lib/aws-ec2';
...
export class RedshiftServerlessWorkgroup extends TrackedConstruct implements IConnectable {
  ...
  constructor(scope: Construct, id: string, props: RedshiftServerlessWorkgroupProps) {
    ...
    const subnetIds = selectedSubnets.subnetIds;
    ...
    this.glueConnection = this.createConnection(selectedSubnets, props);
    this.vpc = props.vpc;
  ...
  /**
     ...
     */
  private createConnection(selectedSubnets: SelectedSubnets, props: RedshiftServerlessWorkgroupProps): CfnConnection {
    const subnet = selectedSubnets.subnets[0];
