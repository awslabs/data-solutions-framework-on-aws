...
def test():
  ...
  green_source_data = sc.parallelize([ 1.68, 2.54, 1.68, 3.0 ]).map(lambda x: (x, )).toDF(schema)

  yellow_source_data = sc.parallelize([ 1.68, 2.54, 1.68, 3.0 ]).map(lambda x: (x, )).toDF(schema)
